...
events = pif.read_events_datarafi(args["file_events"], args["start"], args["end"], freq)
...
if signal.c_on == True:
    ...
    ax_c = plt.subplot2grid((2, 3), (1, 0), colspan=2, sharex=ax_v)
...
if signal.c_on == True:
    ...
    ax_v_m, = ax_v.plot([], [], label="PD neuron" , linewidth=0.4)
    ax_v_l, = ax_v.plot([], [], label="LP neuron", linewidth=0.4)
...
if signal.c_on == True:
    ...
    ax_c.set_ylim(signal.c_min, signal.c_max)
...
if modo==2:
    ...
    ax_i.scatter(events.fPD_fPD, events.lLD_fPD, marker='o', c=events.firstPD, cmap='Reds',  s=1)
    ax_i.scatter(events.fPD_fPD, events.fLP_fPD, marker='o', c=events.firstPD, cmap='Blues', s=1)
...
##### Fit Blue
fit    = np.polyfit( events.fPD_fPD, events.fLP_fPD, 1)
fit_fn = np.poly1d(fit) 
m      = fit[0] #pendiente
yhat   = fit_fn( events.fPD_fPD)                         
ybar   = np.sum( events.fLP_fPD) / len(events.fLP_fPD)    
ssreg  = np.sum( (yhat-ybar)**2)   
sstot  = np.sum( (events.fLP_fPD - ybar)**2)   
R2     = ssreg / sstot
#ax_i.plot(events.fPD_fPD, fit_fn(events.fPD_fPD), linewidth= 0.5, c="midnightblue", label="R2={0:.4f}\tm={1:.4f}".format(R2, m).expandtabs())
ax_i.plot(events.fPD_fPD, fit_fn(events.fPD_fPD), linewidth= 0.5, c="midnightblue", label="LPPD interval\tR2={0:.4f}".format(R2).expandtabs())

##### Fit Red
fit    = np.polyfit( events.fPD_fPD, events.lLD_fPD, 1)
fit_fn = np.poly1d(fit) 
m      = fit[0] #pendiente
yhat   = fit_fn( events.fPD_fPD)                         
ybar   = np.sum( events.lLD_fPD) / len(events.lLD_fPD)    
ssreg  = np.sum( (yhat-ybar)**2)   
sstot  = np.sum( (events.lLD_fPD - ybar)**2)   
R2     = ssreg / sstot
#ax_i.plot(events.fPD_fPD, fit_fn(events.fPD_fPD), linewidth= 0.5, c="firebrick", label="R2={0:.4f}\tm={1:.4f}".format(R2, m).expandtabs())
ax_i.plot(events.fPD_fPD, fit_fn(events.fPD_fPD), linewidth= 0.5, c="firebrick", label="Delay\tR2={0:.4f}".format(R2).expandtabs())

##### Estetica plot fit
ax_i.legend()
ax_i.set_title  ("Dynamical invariant")
ax_i.set_xlabel ("Period (ms)")
ax_i.set_ylabel ("LPPD interval, Delay (ms)")
...
def update(i):
    ...
    if ini_ventana<0:
        pts_avance = 0
        ini_ventana = 0
    elif fin > signal.num_points or (index_event)>=(len(events.firstLP)-2):
        pts_avance = 0
    ...
    ax_v_m.set_data( signal.t[ini_ventana:fin], signal.v_m[ini_ventana:fin] )
    ax_v_l.set_data( signal.t[ini_ventana:fin], signal.v_l[ini_ventana:fin] )
    ax_v.set_xlim( signal.t[ini_ventana], signal.t[fin-1] )
    if signal.c_on == True:
        ax_c_m.set_data( signal.t[ini_ventana:fin], signal.c_m[ini_ventana:fin] )
        ax_c_l.set_data( signal.t[ini_ventana:fin], signal.c_l[ini_ventana:fin] )
        ax_c.set_xlim( signal.t[ini_ventana], signal.t[fin-1] )

    #### EVENTOS
    ref_ini = signal.t[ini_ventana]
    ref_fin = signal.t[ini_ventana+pts_ventana-1]
    ref_event = events.firstLP[index_event+1] #El evento de referencia
...
def update_events(index_event):
    ...
    ax_i_blue_last.set_data ( events.fPD_fPD[index_event], events.fLP_fPD[index_event] )

    #### Actualización de las tres barras sobre el voltaje
    ax_v_event_blue.set_xdata  ( [ events.firstLP [index_event], events.firstPD [index_event] ] )
    ax_v_event_red.set_xdata   ( [ events.lastLP  [index_event], events.firstPD [index_event] ] )
    ax_v_event_black.set_xdata ( [ events.firstLP [index_event], events.firstLP [index_event+1] ] )
    ...
    if modo==1:
        ...
        list_inv_blue_x.append( events.fPD_fPD [index_event] )
        list_inv_blue_y.append( events.fLP_fPD [index_event] )
