...
for i in range(e_min,e_max+1):
	print("Exp ",i)
	cmd = "python3 superpos_from_events_3.py -p %s/exp%d_%s -ws %d -sh n"%(path,i,sufix,wind)
	print(cmd)
	os.system(cmd)
...
for i in range(e_min,e_max+1):
	print("Exp ",i)
	cmd = "python3 superpos_from_events_3.py -p %s/exp%d_%s -ws %d -mean y -sh n"%(path,i,sufix,wind)
	print(cmd)
	os.system(cmd)
