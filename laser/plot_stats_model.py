...
# print(all_df['Cm'])
# n_rows = 2
# n_cols = int(np.ceil(len(all_df.keys())/n_rows))
# print(n_cols, len(all_df.keys())/n_rows)
# fig,ax = plt.subplots(figsize=(30,10),nrows = n_rows, ncols=n_cols)
...
# 	value.boxplot(ax=t_ax)
# 	t_ax.set_title(key)

# plt.tight_layout()
# if save:
# 	plt.savefig(path+name+"_general_stats_grid.pdf",format='pdf')
# plt.close()


# fig,ax = plt.subplots(figsize=(30,15))

# colors = plt.cm.tab20(np.linspace(0,1,len(all_df.keys())))
...
# 	c = rc % n_rows
# 	# print(key, value)
# 	boxprops = dict(linestyle='-', linewidth=3, color=colors[rc])

# 	# value.loc[:,'slope_rep'] = value.loc[:,'slope_rep'].apply(lambda x: abs(x))
# 	# print(value)
# 	# value.iloc[:,:] = value.iloc[:,:].apply(lambda x: x.mean(), axis=0)
# 	# value.iloc[:,:] = value.iloc[:,:].apply(lambda x: (x-x.mean())/ x.std(), axis=0)
# 	# print(value)
# 	from sklearn.preprocessing import StandardScaler
# 	scaler = StandardScaler()
# 	value.iloc[:,:] = scaler.fit_transform(value.iloc[:,:].to_numpy())

# 	# value = (value - value.min()) / (value.max() - value.min())
# 	ax.boxplot(value['duration'], positions=[rc], labels=['duration'], boxprops=boxprops, medianprops=dict(color='k'), showmeans=True)
# 	bp = ax.boxplot(value['amplitude'], positions=[rc+0.25], labels=['amplitude'], boxprops=boxprops, medianprops=dict(color='k'), showmeans=True)
# 	ax.boxplot(value['slope_dep'], positions=[rc+0.5], labels=['depol slope'], boxprops=boxprops, medianprops=dict(color='k'), showmeans=True)
# 	ax.boxplot(value['slope_rep'], positions=[rc+0.75], labels=['repol slope'], boxprops=boxprops, medianprops=dict(color='k'), showmeans=True)
...
# 	# value = (value - value.min()) / (value.max() - value.min())
# 	for i, metric in enumerate(metrics):
# 		boxprops = dict(linestyle='-', linewidth=3, color=colors[i])
# 		if i==1:
# 			labels = [key]
# 		else:
# 			labels = ['']
...
# 		# WARNING: FLIERS!!!!
# 		bp = ax.boxplot(value[metric], positions=[rc+i*0.2+0.2], labels=labels, boxprops=boxprops, medianprops=dict(color='k'), showmeans=True, showfliers=False)
# 		bps.append(bp['boxes'][0])

# ax.set_title(path)


# plt.legend(bps[:len(metrics)],metrics)
# plt.xticks(ha='left')


# plt.tight_layout()

# print("Saving as",path+name+"_general_stats.pdf")
# if save:
# 	plt.savefig(path+name+"_general_stats.pdf",format='pdf')
...
# 	print(min_)
# 	print(max_)
# 	for a_min,a_max in zip(min_,max_):
# 		if a_min > a_max:
...
# if save:
# 	plt.savefig(path+name+"_general_stats_grid_by_metric.pdf",format='pdf')
...
# # Normalize 
# for rc,(key,value) in enumerate(zip(all_df.keys(), all_df.values())):
# 	value.iloc[:,:] = value.iloc[:,:].apply(lambda x: abs(x)/(abs(x).max()), axis=0)

# plot_grid_by_metric()
# if save:
# 	plt.savefig(path+name+"_general_stats_grid_by_metric_norm.pdf",format='pdf')
...
# print(df.groupby('param_name').min())
# print(tabulate(df.groupby('param_name').min(), headers=metrics,tablefmt = 'psql'))
# print(tabulate(df.groupby('param_name').mean(), headers=metrics))
